...
| --- | --- | --- | --- | --- |
| cellRender | Customize appended cell content by panel type (`date` / `month`) | (current, info: { type }) => VueNode \| v-slot:cellRender="{ current, type }" | - | 4.60.0 |
| fullCellRender | Customize full cell content by panel type (`date` / `month`) | (current, info: { type }) => VueNode \| v-slot:fullCellRender="{ current, type }" | - | 4.60.0 |
| ~~dateCellRender~~ | Customize the display of the date cell by setting a scoped slot, the returned content will be appended to the cell. Please use `cellRender` instead | v-slot:dateCellRender="{current: dayjs}" | - |  |
...
| ~~monthCellRender~~ | Customize the display of the month cell by setting a scoped slot, the returned content will be appended to the cell. Please use `cellRender` instead | v-slot:monthCellRender="{current: dayjs}" | - |  |
| ~~monthFullCellRender~~ | Customize the display of the month cell by setting a scoped slot, the returned content will override the cell. Please use `fullCellRender` instead | v-slot:monthFullCellRender="{current: dayjs}" | - |  |
