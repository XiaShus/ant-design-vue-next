...
| percentPosition | 进度数值位置。`align` 控制水平对齐，`type` 控制在进度条内或外 | { align?: `start` \| `center` \| `end`; type?: `inner` \| `outer` } | { align: `end`, type: `outer` } | 4.58.0 |
| rounding | 自定义步骤取整函数 | (step: number) => number | `Math.round` | 4.59.0 |
