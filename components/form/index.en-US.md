...
| variant | Variant for nested data-entry components (antd ≥ 5.13) | `outlined` \| `filled` \| `borderless` \| `underlined` | - | 4.10.0 |
| hideRequiredMark | Hide required mark of all form items. Deprecated, please use `requiredMark={false}` | Boolean | false |  |
| requiredMark | Required mark style. Can be custom render function | boolean \| `optional` \| ((labelNode, info: { required: boolean }) => any) | true | renderProps: 4.73.0 |
...
| hasFeedback | Used with `validateStatus`, this option specifies the validation status icon. Recommended to be used only with `Input`. Object form customizes icons. | boolean \| { icons: [FeedbackIcons](#feedbackicons) } | false | object: 4.40.0 |
...
| labelCol | The layout of label. You can set `span` `offset` to something like `{span: 3, offset: 12}` or `sm: {span: 3, offset: 12}` same as with `<Col>` | [object](/components/grid/#col) |  |  |
| layout | Item layout; overrides Form `layout` for this item | `horizontal` \| `vertical` | - | 4.74.0 |
...
| tooltip | Config tooltip info | string \| slot \| ([TooltipProps](/components/tooltip/#api) & { icon?: any }) |  | 4.0.4；object: 4.73.0 |
| validateDebounce | Debounce delay (ms) for change/blur validation | number | - | 4.75.0 |
