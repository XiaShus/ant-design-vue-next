...
| options | 以配置形式设置子元素 | string\[] \| number[] \| Array&lt;{ label: string value: string disabled?: boolean className?: string style?: CSSProperties }> | - | 4.70.0（className/style） |
| optionType | 用于设置 Radio `options` 类型 | `default` \| `button` | `default` | 3.0.0 |
| block | 选项宽度撑满父容器 | boolean | false | 4.70.0 |
